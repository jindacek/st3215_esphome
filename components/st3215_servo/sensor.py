...
from esphome.const import CONF_ID, UNIT_DEGREE, UNIT_PERCENT, ICON_ROTATE_RIGHT
...
st3215_set_angle = st3215_ns.class_("St3215SetAngleAction", cg.Action)
st3215_rotate = st3215_ns.class_("St3215RotateAction", cg.Action)
st3215_stop = st3215_ns.class_("St3215StopAction", cg.Action)
st3215_move_to_turns = st3215_ns.class_("St3215MoveToTurnsAction", cg.Action)
st3215_move_to_percent = st3215_ns.class_("St3215MoveToPercentAction", cg.Action)
...
@cg.register_action("st3215_servo.set_angle", st3215_set_angle, cv.Schema({
...
@cg.register_action("st3215_servo.rotate", st3215_rotate, cv.Schema({
    cv.Required(CONF_ID): cv.use_id(St3215Servo),
...
@cg.register_action("st3215_servo.stop", st3215_stop, cv.Schema({
    cv.Required(CONF_ID): cv.use_id(St3215Servo),
}))
...
@cg.register_action("st3215_servo.move_to_turns", st3215_move_to_turns, cv.Schema({
    cv.Required(CONF_ID): cv.use_id(St3215Servo),
    cv.Required("turns"): cv.templatable(cv.float_),
    cv.Optional("speed", default=600): cv.templatable(cv.int_),
}))
async def turns_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
...
@cg.register_action("st3215_servo.move_to_percent", st3215_move_to_percent, cv.Schema({
    cv.Required(CONF_ID): cv.use_id(St3215Servo),
    cv.Required("percent"): cv.templatable(cv.float_),
    cv.Optional("speed", default=600): cv.templatable(cv.int_),
}))
async def percent_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
    perc = await cg.templatable(config["percent"], args, cg.float_)
    speed = await cg.templatable(config["speed"], args, cg.int_)
    cg.add(var.set_percent(perc))
    cg.add(var.set_speed(speed))
    return var
