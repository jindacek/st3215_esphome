...
void St3215Servo::setup() {
  ESP_LOGI(TAG, "ST3215 servo setup, id=%u", servo_id_);
}
...
void St3215Servo::send_packet_(uint8_t inst, const std::vector<uint8_t> &params) {
  uint8_t len = params.size() + 2;  // inst + checksum
...
void St3215Servo::update_turn_counter_(float new_angle) {
  ...
  if (delta > max_angle_ / 2.0f) {
    current_turns_ -= 1.0f;
  } else if (delta < -max_angle_ / 2.0f) {
    current_turns_ += 1.0f;
  }
  last_angle_ = new_angle;
}
...
void St3215Servo::move_to_turns(float turns, int speed) {
  float target_turns = turns;
  float frac = fmodf(target_turns, 1.0f);
  if (frac < 0) frac += 1.0f;
  float target_angle = frac * max_angle_;

  // Use position mode for final landing.
  set_angle(target_angle, speed);

  // NOTE: For long multi-turn moves, prefer rotate() + stop logic
  // in an automation while watching the turns sensor.
}

void St3215Servo::move_to_percent(float percent, int speed) {
  if (turns_full_open_ <= 0.0f) return;
  percent = clamp(percent, 0.0f, 100.0f);
  float target_turns = (percent / 100.0f) * turns_full_open_;
  move_to_turns(target_turns, speed);
}
...
// ---- Actions ----
void St3215SetAngleAction::play(float x, int y) {
  parent_->set_angle(angle_.value(x), speed_.value(y));
}

void St3215RotateAction::play(int x) {
  parent_->rotate(cw_, speed_.value(x));
}

void St3215MoveToTurnsAction::play(float x, int y) {
  parent_->move_to_turns(turns_.value(x), speed_.value(y));
}

void St3215MoveToPercentAction::play(float x, int y) {
  parent_->move_to_percent(percent_.value(x), speed_.value(y));
}
