...
void St3215Servo::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "Safety: max_run=%u ms, stall_timeout=%u ms, stall_grace=%u ms, stall_delta=%.4f turns",
                max_run_time_ms_, stall_timeout_ms_, stall_grace_ms_, stall_min_delta_turns_);
}
...
void St3215Servo::update() {
  ...
  // Hlídá dvě věci:
  // 1) jedna jízda nesmí trvat déle než max_run_time_ms_
  // 2) když má servo jet, ale enkodér se prakticky nehýbe, jde o zaseknutí
  if (moving_ && !calibration_active_) {
    uint32_t now_safety = millis();

    // kdyby se z nějakého důvodu monitor nenastavil při rotate(), inicializujeme ho tady
    ...
    if (moved >= stall_min_delta_turns_) {
      ...
      safety_stop_("Chyba: zaseknutá roleta");
      return;
    }
  }
...
void St3215Servo::safety_stop_(const std::string &reason) {
  ...
  stop();
  set_torque(false);   // při bezpečnostní chybě uvolníme servo, aby dál netlačilo
