    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_TORQUE): switch.switch_schema(St3215TorqueSwitch),
        }
    )
...
async def to_code(config):
    ...
    if CONF_TORQUE in config:
        sw = cg.new_Pvariable(config[CONF_TORQUE][CONF_ID], var)
        await switch.register_switch(sw, config[CONF_TORQUE])
