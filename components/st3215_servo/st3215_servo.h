...
// Feetech/SCServo-compatible constants.
// If your servo firmware uses different addresses, adjust here.
static const uint8_t INST_PING  = 0x01;
static const uint8_t INST_READ  = 0x02;
static const uint8_t INST_WRITE = 0x03;

// Control table addresses (STS3215 / ST3215)
static const uint8_t ADDR_GOAL_POSITION_L    = 0x2A;  // 42
static const uint8_t ADDR_GOAL_TIME_L        = 0x2C;  // 44
static const uint8_t ADDR_GOAL_SPEED_L       = 0x2E;  // 46
static const uint8_t ADDR_PRESENT_POSITION_L = 0x38;  // 56
static const uint8_t ADDR_PRESENT_SPEED_L    = 0x3A;  // 58
...
class St3215Servo : public PollingComponent, public uart::UARTDevice {
 public:
  ...
  float get_current_turns() const { return current_turns_; }
 ...
 protected:
  ...
  float max_angle_{240.0f};        // degrees per mechanical sweep
  float turns_full_open_{0.0f};    // user calibration, 0 disables percent sensor
...
class St3215SetAngleAction : public Action<float, int> {
 public:
  ...
  void play(float x, int y) override;
